...
export const store = createStore<StoreModel>({
  ...
  getNews: thunk(async (actions, payload) => {
    try {
      const { data }: { data: ResponseData } = await instance.get(
        `/top-headlines?${
          payload.filterBy
            ? `country=${payload.country}`
            : `${
                payload.country ? `country=${payload.country}` : payload.query ? `q=${payload.query}` : ''
              }&pageSize=10${payload.page ? `&page=${payload.page}` : ''}`
        }`
      );

      if (payload.filterBy) {
        const filteredData = data.articles.filter((article: IArticle) => {
          return payload.filterBy === 'source name'
            ? article.source.name
              ? article.source.name.toLocaleLowerCase().includes(payload.query ? payload.query.toLocaleLowerCase() : '')
              : false
            : article[payload.filterBy]
            ? article[payload.filterBy]
                .toLocaleLowerCase()
                .includes(payload.query ? payload.query.toLocaleLowerCase() : '')
            : false;
        });
        payload.country
          ? actions.setNews({ news: filteredData, country: payload.country, filtered: true })
          : actions.setNews({ news: filteredData, filtered: true });
      } else {
        payload.country
          ? actions.setNews({ news: data.articles, country: payload.country })
          : actions.setNews({ news: data.articles });
      }
    } catch (e) {
      console.log(e);
    }
